/*
Given a array of numbers representing the stock prices of a company in chronological order,
write a function that calculates the maximum profit you could have made from buying and selling that stock once. You must buy before you can sell it.

For example, given [9, 11, 8, 5, 7, 10], you should return 5, since you could buy the stock at 5 dollars and sell it at 10 dollars.
*/

function maxStockProfit(stockPrices) {
    if (stockPrices.length <= 1) {
        return 0;
    }

    let minPrice = stockPrices[0];
    let maxPriceDiff = 0; // Set this to Number.NEGATIVE_INFINITY for the case where one must buy and sell once.

    for (let i = 1; i < stockPrices.length; i++) {
        maxPriceDiff = stockPrices[i] - minPrice > maxPriceDiff ? stockPrices[i] - minPrice : maxPriceDiff;
        minPrice = stockPrices[i] < minPrice ? stockPrices[i] : minPrice;
    }

    return maxPriceDiff;
}

console.log(maxStockProfit([9, 11, 8, 5, 7, 10]));